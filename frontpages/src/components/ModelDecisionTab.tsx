...
import { Brain, Activity, XCircle, PlusCircle, MinusCircle, TrendingUp } from 'lucide-react';
                      ...
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2">
                          ...
                          {action.status === 'rejected' ? (
                            ...
                            <>
                              {action.action === 'open_long' || action.action.toLowerCase().includes('long') ? (
                              ...
                              ) : action.action === 'open_short' || action.action.toLowerCase().includes('short') ? (
                                <div className="flex items-center gap-1.5 px-2 py-0.5 bg-rose-500/20 text-rose-400 text-xs rounded uppercase border border-rose-500/30">
                                  <MinusCircle className="w-3 h-3" />
                                  <span>{action.action.replace('_', ' ')}</span>
                                </div>
                              ) : action.action === 'close_position' || action.action.toLowerCase().includes('close') ? (
                                <div className="flex items-center gap-1.5 px-2 py-0.5 bg-orange-500/20 text-orange-400 text-xs rounded uppercase border border-orange-500/30">
                                  <MinusCircle className="w-3 h-3" />
                                  <span>{action.action.replace('_', ' ')}</span>
                                </div>
                              ) : (
